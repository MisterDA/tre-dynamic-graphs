...
\begin{algorithm}[H]
  ...
  \(run \leftarrow \top\)\;
  % \While{\(\exists w \in V_G\) tel que \(location(w)\) n'est pas
  % définie}{
  \While{\(run = \top\)}{
    \(run \leftarrow \bot\)\;
...
\begin{figure}[!h]
  ...
  \subfloat[\(G_3\)]{
    \begin{tikzpicture}[node/.style={circle, draw, minimum size=1cm]},
      ...
      \path[->] (e) edge (f);
    \end{tikzpicture}}
  \caption{Graphe dynamique \(G = (G_0, G_1, G_2, G_3)\)}\label{graph_bug}
\end{figure}

Il y a un problème lors de la construction de l'arbre des plus courts
chemins~\ref{tree_bug}. Dans \(T_2\), on découvre pour la première
...
\begin{figure}[!t]
  ...
  \subfloat[\(T_3\)]{
    \begin{tikzpicture}[node/.style={circle, draw, minimum size=1cm]},
      ...
      \path[->] (c) edge (b);
    \end{tikzpicture}}
  \caption{Arbre des plus courts chemins de \(G\)}\label{tree_bug}
\end{figure}
...
valides. \textcolor{red}{FIXME:\@ vérifier l'implémentation sur ce
  graphe. La fonction \(neighbours\) n'est pas si triviale à coder.}
